...
class Configurable(ConfigMetaHelper):
    ...
    def __init__(self, name=None, **kwargs):
        ...
        if 'parent' in kwargs:
            ...
            if isinstance(parent,
                          basestring if sys.version_info[0] == 2 else str):
    ...
    def name(self):
        if not self._name:
            raise AttributeError('{!r} instance was not named yet'.format(
                type(self).__name__))
        return self._name
    ...
    def name(self, value):
        ...
        if not isinstance(value, basestring
                          if sys.version_info[0] == 2 else str) or not value:
            raise TypeError('expected string, got {} instead'.format(
                type(value).__name__))
    ...
    def __repr__(self):
        ...
        try:
            ...
            pass  # no name
        args.extend(
            '{}={!r}'.format(*item) for item in self._properties.items())
    ...
    def merge(self, other):
        ...
        if hasattr(self, 'name') and (self.name != other.name):
            raise ValueError(
                'cannot merge configurables with different names ({} and {})'.
        ...
        for name in other._descriptors:
            ...
            try:
                setattr(
                    self, name, self._descriptors[name].__merge__(
                        self, type(self), getattr(other, name)))
            except ValueError as err:
                raise ValueError(
                    'conflicting settings for property {} of {}: {}'.format(
                        name, self.name
                        if hasattr(self, 'name') else type(self).__name__,
                        str(err)))
...
def makeConfigurableClass(name, **namespace):
    ...
    namespace.update(
        {pname: Property(*pargs)
         for pname, pargs in properties.items()})
