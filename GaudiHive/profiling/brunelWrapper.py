...
def createParser():
    ...
    parser.add_option('--nthreads', help='Number of threads', dest='nthreads', default=10)
    ...
    parser.add_option('-v', help='Verbosity level', dest='verbosity', default=5)
    parser.add_option('--exec', help='ExecuteWorkflow',dest='execbrunel', action='store_true', default=False)
    parser.add_option('--bg', help='Launch in background',dest='bg', action='store_true', default=False)
    parser.add_option('--scale', help='Scale Algorithms time',dest='scale', default=1)    
...
def replaceValues(cfg_name,n,eif,aif,nthreads,scale,clone,dumpQueues,verbosity):
    ...
    if scale != 1:
      scale_s="_s%s"%scale
    newcfgname="measurement_%s_n%s_eif%s_aif%s_nthreads%s_c%s_dq%s%s%s.py" %(cfg_name,n,eif,aif,nthreads,clone,dumpQueues,scale_s,verb)
