#define  GAUDIHIVE_HiveEventLoopMgr_old_CPP
...
// For concurrency
#include "GaudiHive/HiveEventLoopMgr_old.h"
...
DECLARE_SERVICE_FACTORY(HiveEventLoopMgr_old)
...
class HiveAlgoTask_old : public tbb::task {
public:
  HiveAlgoTask_old(IAlgorithm* algorithm, EventSchedulingState* scheduler): m_algorithm(algorithm), m_scheduler(scheduler){};
  tbb::task* execute();
  IAlgorithm* m_algorithm;
  EventSchedulingState* m_scheduler;
};

tbb::task* HiveAlgoTask_old::execute() {
...
//--------------------------------------------------------------------------------------------
HiveEventLoopMgr_old::HiveEventLoopMgr_old(const std::string& nam, ISvcLocator* svcLoc)
...
// Standard Destructor
//--------------------------------------------------------------------------------------------
HiveEventLoopMgr_old::~HiveEventLoopMgr_old()   {
...
//--------------------------------------------------------------------------------------------
StatusCode HiveEventLoopMgr_old::initialize()    {
...
//--------------------------------------------------------------------------------------------
StatusCode HiveEventLoopMgr_old::reinitialize() {
...
//--------------------------------------------------------------------------------------------
StatusCode HiveEventLoopMgr_old::stop()    {
  if ( ! m_endEventFired ) {
...
//--------------------------------------------------------------------------------------------
StatusCode HiveEventLoopMgr_old::finalize()    {
  StatusCode sc;
...
//--------------------------------------------------------------------------------------------
StatusCode HiveEventLoopMgr_old::executeRun( int maxevt )    {
  StatusCode  sc;
  // initialize the base class
...
StatusCode HiveEventLoopMgr_old::nextEvent(int maxevt)   {
...
StatusCode HiveEventLoopMgr_old::getEventRoot(IOpaqueAddress*& refpAddr)  {
  refpAddr = 0;
...
void
HiveEventLoopMgr_old::find_dependencies() {
    ...
    for (ListAlg::iterator ita = m_topAlgList.begin(); ita != m_topAlgList.end(); ita++ ) {
      ...
      try {
        ...
        if ( (dependencies_missing == 0) &&
          ...
          tbb::task* t = new( tbb::task::allocate_root() ) HiveAlgoTask_old((*ita), &event_state);
          tbb::task::enqueue( *t);
