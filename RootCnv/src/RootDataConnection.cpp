...
namespace {
  std::array<char, 256> init_table() {
    ...
    return table; // cppcheck-suppress uninitvar; false positive
  }
...
StatusCode RootDataConnection::connectRead() {
  ...
  // Compare uuids in a case insensitive way
  if ( !need_fid &&
       !std::ranges::equal( fid, m_fid, []( char a, char b ) { return std::tolower( a ) == std::tolower( b ); } ) ) {
