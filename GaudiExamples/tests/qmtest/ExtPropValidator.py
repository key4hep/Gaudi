def validate(stdout, stderr, result, causes):
    expected = {
        ...
        'MapStringDouble': {'aa': 0.1,
                            'bb': 0.2,
                            'cc': 3.0},
        'MapStringInt': {'a': 1, 'b': 2, 'c': 3},
        'MapStringString': {'a': 'sddsgsgsdgdggf',
                            'b': 'sddsgsgsdgdggf',
                            'c': 'sddsgsgsdgdggf'},
        'MapStringVectorOfDoubles': {'aaa': [1.0, 2.0, 3.0],
                                     'bbb': [1.0, 2.0, 3.0],
                                     'ccc': [0.1, 0.2, 0.3]},
        'MapStringVectorOfInts': {'aaa': [1, 2, 3],
                                  'bbb': [4, 5, 6],
                                  'ccc': [7, 8, 9]},
        'MapStringVectorOfStrings': {'aaa': ['a', 'b', 'c'],
                                     'bbb': ['a', 'b', 'c'],
                                     'ccc': ['a', 'b', 'c']},
        'MapUIntString': {0: 'UZero', 1: 'UOne', 2: 'UTwo'},
        'PairDD': (3141.592, 2.1828183),
        'PairII': (3, 2),
        'VectorOfPairsDD': [(0.0, 1.0),
                            (1.0, 2.0),
                            (2.0, 3.0),
                            (3.0, 4.0),
                            (4.0, 5.0),
    ...
    try:
        ...
        if properties != expected:
            ...
            pp = PrettyPrinter()
            result['GaudiTest.properties.expected'] = \
                result.Quote(pp.pformat(expected))
            result['GaudiTest.properties.found'] = \
                result.Quote(pp.pformat(properties))
            pkeys = set(properties)
            ekeys = set(expected)
            missing = sorted(ekeys - pkeys)
            if missing:
                result['GaudiTest.properties.missing_keys'] = \
                    result.Quote(pp.pformat(missing))
            extra = sorted(pkeys - ekeys)
            if extra:
                result['GaudiTest.properties.extra_keys'] = \
                    result.Quote(pp.pformat(extra))
            mismatch = sorted([k
                               for k in ekeys.intersection(pkeys)
                               if expected[k] != properties[k]])
            if mismatch:
                result['GaudiTest.properties.mismatch_keys'] = \
                    result.Quote(pp.pformat(mismatch))
