...
int main() {
  ...
  // - main configuration
  {
    SmartIF<IProperty> prop{app};
    prop->setProperty( "JobOptionsType", "NONE" );
    prop->setProperty( "EventLoop", "Gaudi::Examples::AsyncEventLoopMgr/AsyncEventLoopMgr" );
    prop->setProperty( "OutputLevel", "3" ); // INFO OutputLevel
    prop->setProperty( "TopAlg", "['GaudiTesting::SleepyAlg/Alg1']" );
  }
  // - start minimal services
  SmartIF<IStateful> sm{app};
  sm->configure();
  // - prepare job configuration
  {
    SmartIF<ISvcLocator> sloc{app};
    auto                 jos = sloc->service<IJobOptionsSvc>( "JobOptionsSvc" );
    jos->addPropertyToCatalogue( "Alg1", Gaudi::Property<int>( "SleepTime", 1 ) );
    // use DEBUG output for AsyncEventLoopMgr
    jos->addPropertyToCatalogue( "AsyncEventLoopMgr", Gaudi::Property<int>( "OutputLevel", 2 ) );
  }
  // - get ready to process events
  sm->initialize();
  sm->start(); // this starts the AsyncEventLoopMgr processing thread

  // - this MsgStream is useful to have uniform printout
  MsgStream log( SmartIF<IMessageSvc>{app}, "<main>" );
  ...
  sm->stop(); // this stops the AsyncEventLoopMgr processing thread
  sm->finalize();
  sm->terminate();
}
