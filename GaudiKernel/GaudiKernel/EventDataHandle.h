...
namespace Gaudi
{
  ...
  /// store. This is internal machinery which clients can safely ignore.
  class EventDataHandle : public Gaudi::v2::DataHandle
  {
  public:
  ...
  protected:
    ...
    public:
      ...
      Metadata( AccessMode access ) : m_access{access} {}

      // Identifier for the whiteboard implementation that we use
      std::string whiteBoard() const final override { return "IDataProviderSvc"; }
  ...
  public:
    ...
    /// Access the data for the current event context. Returns a const-reference
    /// to the target data, automatically building Range<>s from containers.
  ...
  private:
    ...
    void putImpl( const EventContext& /* ctx */, std::unique_ptr<DataObject>&& ptr ) const
    {
      // TODO: Introduce and use an EventContext-aware whiteboard interface
      auto sc = m_whiteBoard->registerObject( targetKey().key(), ptr.release() );
      if ( sc.isFailure() ) {
