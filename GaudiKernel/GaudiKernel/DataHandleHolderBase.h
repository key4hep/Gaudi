...
class GAUDI_API DataHandleHolderBase : public extends<BASE, IDataHandleHolder>
{
  ...
  protected:
    ...
    void collectExplicitDeps() {
      ...
      if(m_explicitDepsCollected) {
        throw GaudiException( "Explicit dependencies may only be collected once",
                              this->name(),
                              StatusCode::FAILURE );
      }
    ...
    StatusCode handleCircularDeps( CircularDepHandler&& circularDepHandler ) {
      ...
      if(!m_explicitDepsCollected) {
        throw GaudiException( "Explicit dependencies must be collected first",
                              this->name(),
                              StatusCode::FAILURE );
      }
  ...
  private:
    ...
    std::vector<Gaudi::DataHandle*> selectLegacyHandles( Gaudi::DataHandle::Mode mode ) const
    {
      ...
      std::copy_if( std::begin( m_legacyHandles ),
                    std::end( m_legacyHandles ),
