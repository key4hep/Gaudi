...
BOOST_AUTO_TEST_CASE( test_static_counter_histos, *boost::unit_test::tolerance( 1e-14 ) ) {
...

  {
    ...
    for ( unsigned int i = 0; i < 5; i++ ) BOOST_TEST( nlohmann::json( histo1d[i] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d[2] ).at( "title" ) == "A Gaudi 1D histogram - number 2" );
    ++histo1d[3][-10.0]; // fill the first (non-overflow) bin
    BOOST_TEST( nlohmann::json( histo1d[3] ).at( "bins" )[1] == 2 );
  }

  {
    ...
    ++histo1d["two"][-10.0];
    ++histo1d["three"][-10.0];
    BOOST_TEST( nlohmann::json( histo1d["one"] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d["two"] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d["three"] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d["four"] ).at( "bins" )[1] == 0 );
    BOOST_TEST( nlohmann::json( histo1d["two"] ).at( "title" ) == "A Gaudi 1D histogram - number two" );
    ++histo1d["three"][-10.0]; // fill the first (non-overflow) bin
    BOOST_TEST( nlohmann::json( histo1d["three"] ).at( "bins" )[1] == 2 );
  }

  {
    ...
    for ( unsigned int i = 0; i < 7; i++ )
      BOOST_TEST( nlohmann::json( histo2dw[i] ).at( "bins" )[( 1 + 21 + 1 ) + 1] == 0.25 );
    for ( unsigned int i = 0; i < 7; i++ ) histo2dw[i][{ -10.0, -10.0 }] += 0.5; // fill the first (non-overflow) bin
    for ( unsigned int i = 0; i < 7; i++ )
      BOOST_TEST( nlohmann::json( histo2dw[i] ).at( "bins" )[( 1 + 21 + 1 ) + 1] == 0.75 );
  }

  {
    ...
    for ( unsigned int i = 0; i < 5; i++ ) ++histo1d[i][-10.0]; // fill the first (non-overflow) bin
    for ( unsigned int i = 0; i < 5; i++ ) BOOST_TEST( nlohmann::json( histo1d[i] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d[3] ).at( "title" ) ==
                "Title number 3 of Histogram arrays of 5 histograms in total" );
    ++histo1d[3][-10.0]; // fill the first (non-overflow) bin
    BOOST_TEST( nlohmann::json( histo1d[3] ).at( "bins" )[1] == 2 );
  }

  {
    ...
    BOOST_TEST( nlohmann::json( histo1d["three"] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d["four"] ).at( "bins" )[1] == 0 );
    BOOST_TEST( nlohmann::json( histo1d["three"] ).at( "title" ) == "Title number three of Histogram arrays" );
    ++histo1d["three"][-10.0]; // fill the first (non-overflow) bin
    BOOST_TEST( nlohmann::json( histo1d["three"] ).at( "bins" )[1] == 2 );
  }

  {
    ...
    BOOST_TEST( nlohmann::json( histo1d[{ 1, "one" }] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d[{ 2, "two" }] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d[{ 3, "three" }] ).at( "bins" )[1] == 1 );
    BOOST_TEST( nlohmann::json( histo1d[{ 4, "four" }] ).at( "bins" )[1] == 0 );
    BOOST_TEST( nlohmann::json( histo1d[{ 3, "three" }] ).at( "title" ) == "Title three (3)" );
    ++histo1d[{ 3, "three" }][-10.0]; // fill the first (non-overflow) bin
    BOOST_TEST( nlohmann::json( histo1d[{ 3, "three" }] ).at( "bins" )[1] == 2 );
  }
}
