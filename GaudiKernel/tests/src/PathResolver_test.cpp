...
#  endif
  ...
  if ( fileLocation == "" ) {
    ...
    if ( fileLocation != ref1 ) {
      std::cout << "  => ERROR: should have been \"" << ref1 << "\"" << std::endl;
      err++;
    }
  } else {
    std::cout << "  => ERROR: could not remove local path from fileLocation" << std::endl;
    err++;
  }
  ...
  if ( fileLocation == "" ) {
    ...
    if ( fileLocation != ref2 ) {
      std::cout << "  => ERROR: should have been \"" << ref2 << "\"" << std::endl;
      err++;
    }
  } else {
    std::cout << "  => ERROR: could not remove local path from fileLocation" << std::endl;
  ...
  if ( fileLocation == "" ) {
    ...
    if ( fileLocation != ref3 ) {
      std::cout << "  => ERROR: should have been \"" << ref3 << "\"" << std::endl;
      err++;
    }
  } else {
    std::cout << "  => ERROR: could not remove local path from fileLocation" << std::endl;
  ...
  if ( fileLocation == "" ) {
    ...
    std::cout << "  => ERROR: should have been \"" << ref4 << "\"  was \"" << fileLocation << "\"" << std::endl;
    err++;
  }

  std::cout << "*** PathResolver_test ends (" << ( err == 0 ? "OK" : "ERROR" ) << ") ***" << std::endl;
