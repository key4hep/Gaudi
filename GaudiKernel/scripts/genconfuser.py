...
def getConfigurableUsers(modulename, root, mayNotExist=False):
    ...
    if not os.path.exists(os.path.join(moddir, shortmodname) + ".py"):
        msg = f"Module {modulename} does not exist"
        if mayNotExist:
            logging.verbose(msg)
        else:
            logging.error(msg)
    ...
    try:
        ...
        exec(f"import {shortmodname} as mod", g, l)
    finally:
...
def main():
    ...
    if cus:
        ...
        # header
        output = f"""##  -*- ascii -*-
# db file automatically generated by {parser.prog} on: {time.asctime()}
"""
        ...
        output = (
            f"# db file automatically generated by {parser.prog} on: {time.asctime()}\n"
            f"# No ConfigurableUser specialization in {package_name}\n"
        )
    else:
