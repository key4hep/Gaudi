...
/// Unregister object from the data store.
StatusCode TsDataSvc::unregisterObject (DataObject* pParentObj,
                                      const std::string& objectPath) {
  if ( checkRoot() )  {
    try   {
...
StatusCode TsDataSvc::retrieveEntry(RegEntry* parentObj,
...
StatusCode TsDataSvc::retrieveObject(IRegistry* pRegistry,
                                   const std::string& path,
                                   DataObject*& pObject)   {
  STD_LOCK_GUARD_MACRO
  pObject = 0;
...
/// Retrieve object identified by its directory from the data store.
StatusCode TsDataSvc::findObject(IRegistry* pRegistry,
                               const std::string& path,
                               DataObject*& pObject)   {
  STD_LOCK_GUARD_MACRO
  pObject = 0;
...
StatusCode TsDataSvc::findObject(const std::string& path,
                               DataObject*& pObject)   {
  STD_LOCK_GUARD_MACRO
  pObject = 0;
  if ( checkRoot() )  {
...
/// Update object.
StatusCode TsDataSvc::updateObject(DataObject* parent,
                                 const std::string& updatePath)   {
  DataObject* pObject = 0;
...
/// Add a link to another object.
StatusCode TsDataSvc::linkObject(const std::string& from,
                               const std::string& objPath,
                               DataObject* to)    {
  STD_LOCK_GUARD_MACRO
  DataObject* pO = 0;
...
/// Remove a link to another object.
StatusCode TsDataSvc::unlinkObject(const std::string& from,
                                 const std::string& objPath)    {
  DataObject* pObject = 0;
...
/// Remove a link to another object.
StatusCode TsDataSvc::unlinkObject(DataObject* from,
                                 const std::string& objPath)    {
  if ( checkRoot() )  {
