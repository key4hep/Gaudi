...
long System::basePriority( InfoType fetch, long pid ) {
  ...
  if ( fetch != NoFetch && getProcess()->query( pid, ProcessBasics, &info ) ) return info.BasePriority;
  return 0;
}
...
long System::parentID( InfoType fetch, long pid ) {
  PROCESS_BASIC_INFORMATION info;
  if ( fetch != NoFetch && getProcess()->query( pid, ProcessBasics, &info ) ) return info.InheritedFromUniqueProcessId;
  return 0;
}
...
long System::affinityMask( InfoType fetch, long pid ) {
  PROCESS_BASIC_INFORMATION info;
  if ( fetch != NoFetch && getProcess()->query( pid, ProcessBasics, &info ) ) return info.AffinityMask;
  return 0;
}
...
long System::exitStatus( InfoType fetch, long pid ) {
  PROCESS_BASIC_INFORMATION info;
  if ( fetch != NoFetch && getProcess()->query( pid, ProcessBasics, &info ) ) return info.ExitStatus;
  return -2;
}
...
long System::priorityBoost( InfoType fetch, long pid ) {
  long info;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &info ) ) return info;
  return -2;
}
...
long System::numPageFault( InfoType fetch, long pid ) {
  VM_COUNTERS info;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &info ) ) return info.PageFaultCount;
  return -2;
}
...
long System::pagefileUsage( MemoryUnit unit, InfoType fetch, long pid ) {
  VM_COUNTERS info;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &info ) ) return adjustMemory( unit, info.PagefileUsage );
  return -2;
}
...
long System::pagefileUsageLimit( MemoryUnit unit, InfoType fetch, long pid ) {
  POOLED_USAGE_AND_LIMITS quota;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &quota ) ) {
    if ( long( quota.PagefileLimit ) < 0 ) return -1; // LONG_MAX;
...
long System::mappedMemory( MemoryUnit unit, InfoType fetch, long pid ) {
  VM_COUNTERS info;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &info ) ) return adjustMemory( unit, info.WorkingSetSize );
  return -2;
}
...
long System::virtualMemory( MemoryUnit unit, InfoType fetch, long pid ) {
  VM_COUNTERS info;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &info ) ) return adjustMemory( unit, info.VirtualSize );
  return -2;
}
...
long System::virtualMemoryPeak( MemoryUnit unit, InfoType fetch, long pid ) {
  VM_COUNTERS info;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &info ) ) return adjustMemory( unit, info.PeakVirtualSize );
  return -2;
}
...
long System::virtualMemoryLimit( MemoryUnit unit, InfoType fetch, long pid ) {
  QUOTA_LIMITS quota;
  if ( fetch != NoFetch && getProcess()->query( pid, fetch, &quota ) ) {
    if ( long( quota.PagefileLimit ) == -1 ) return -1; // LONG_MAX;
    return adjustMemory( unit, quota.PagefileLimit );
  }
  return 0;
}
