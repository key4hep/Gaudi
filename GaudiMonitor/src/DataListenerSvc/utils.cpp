...
bool apmon_utils::urlModified(char *url, char *lastModified) throw(runtime_error) {
...
#ifndef WIN32
  sprintf(temp_filename, "/tmp/apmon_http%ld", mypid);
#else
  ...
  if(tmp == NULL)
	  tmp = "c:";
  sprintf(temp_filename, "%s\\apmon_http%ld", tmp, mypid);
#endif
  /* get the HTTP header and put it in a temporary file */
  httpRequest(url, (char*)"HEAD", temp_filename);
...
throw(runtime_error) {
  ...
  if(h==NULL) {
    free(request);
    sprintf(msg,"[ httpRequest() ] Unknown host: %s ", hostname);
  ...
  if(rc<0) {
    free(request);
    close(sd);
    sprintf(msg, "%s: cannot bind port TCP %u", url,port);
    throw runtime_error(msg);
  }
  */
...
char *apmon_utils::findIP(char *address) throw(runtime_error) {
  ...
  for (j = 0; j < strlen(address); j++) 
    ...
    if (!isIP) {  // the user provided a hostname, find the IP
      ...
      if (he == NULL) {
	char tmp_msg[40];
	sprintf(tmp_msg, "[ findIP() ] Invalid destination address %s", address);
