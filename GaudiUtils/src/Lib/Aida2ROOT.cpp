...
namespace {
    ...
    Out *a2r_cast(In* aida) {
        using Base = typename std::conditional<std::is_const<Out>::value,
                                               const Gaudi::HistogramBase,
                                               Gaudi::HistogramBase>::type;
...
{
  return a2r_cast<TProfile2D>( aida );
}
