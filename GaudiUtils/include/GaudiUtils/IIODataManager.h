...
namespace Gaudi {
  ...
  public:
    ...
    IDataConnection( const IInterface* own, std::string nam ) : m_name( std::move( nam ) ), m_owner( own ) {}
    /// Standard destructor
  ...
  public:
    ...
    /// Open data stream in write mode
    virtual StatusCode connectWrite( IDataConnection* con, IoType mode = Connection::CREATE,
                                     const std::string& doctype = "UNKNOWN" ) = 0;
    /// Release data stream
