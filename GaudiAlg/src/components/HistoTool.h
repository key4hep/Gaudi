...
public:
  ...
  AIDA::IHistogram1D* plot1D( const double value, const std::string& title, const double low, const double high,
                              const unsigned long bins = 100, const double weight = 1.0 ) const override

  {
    return GaudiHistoTool::plot1D( value, title, low, high, bins, weight );
  };
...
public:
  ...
  AIDA::IHistogram1D* histo1D( const std::string& title ) const override { return GaudiHistoTool::histo1D( title ); }

  // access the EXISTING 1D histogram by ID
  AIDA::IHistogram1D* histo1D( const HistoID& ID ) const override { return GaudiHistoTool::histo1D( ID ); }

  // access the EXISTING 2D histogram by title
  AIDA::IHistogram2D* histo2D( const std::string& title ) const override { return GaudiHistoTool::histo2D( title ); }

  // access the EXISTING 2D histogram by ID
  AIDA::IHistogram2D* histo2D( const HistoID& ID ) const override { return GaudiHistoTool::histo2D( ID ); }

  // access the EXISTING 3D histogram by title
  AIDA::IHistogram3D* histo3D( const std::string& title ) const override { return GaudiHistoTool::histo3D( title ); }

  // access the EXISTING 3D histogram by ID
  AIDA::IHistogram3D* histo3D( const HistoID& ID ) const override { return GaudiHistoTool::histo3D( ID ); }

  // check the existence AND validity of the histogram with given title
  bool histoExists( const std::string& title ) const override { return GaudiHistoTool::histoExists( title ); }
