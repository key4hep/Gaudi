...
// flow. We need to run profiling  throw Intel Apmplifier amplxe-cl command
// line tool.
...
// ## Private attributes.  
private:
  // Stack for store current component(algorithm) chain with usefull 
...
private: 
  ...
  // The String dilimeter between sequences/alogirthms names in
...
IntelProfilerAuditor::IntelProfilerAuditor(const std::string& name,
  ...
  declareProperty("ComponentsForTaskTypes", m_algs_for_tasktypes,
    ...
    "By default all algorithms have a correspondinding task type.");
...
StatusCode IntelProfilerAuditor::initialize() {
  ...
  // Create a profilier domain for detection of slow events.
...
void IntelProfilerAuditor::before(StandardEventType type, INamedInterface* i) {
  ...
  if (isRunning()) {
    ...
    if (hasIncludes()) {
      // If the profiling is not running and  "includes" is explicitly definded ...
      if (isIncluded(name)) {
