...
class PropertyEntry(object):
    ...
    def __init__(self, prop):
        ...
        if isinstance(prop, GaudiHandleProperty):
            self._value = prop.value()  # do nothing for ATLAS' handles
        elif isinstance(prop, GaudiHandleArrayProperty):
            self._value = prop.value()  # do nothing for ATLAS' handles
        else:
...
class iProperty(object):
    ...
    def __setattr__(self, name, value):
        ...
        if hasattr(value, "toStringProperty"):
            ...
            # We want a reproducible (sorted) representation in the catalogue
    ...
    def __getattr__(self, name):
        ...
        if ip:
            ...
            if isinstance(prop, StringProperty):
                return prop.value()
            elif isinstance(prop, StringPropertyRef):
                return prop.value()
            try:
