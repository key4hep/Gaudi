...
/// Constructor
GaudiPython::TESSerializer::TESSerializer( IDataProviderSvc* svc, IAddressCreator* ac )
  : m_TES(svc)
{
...
void GaudiPython::TESSerializer::addItem(Items& itms, const std::string& descriptor)   {
  // supports # notation
  int level = 0;
...
void GaudiPython::TESSerializer::checkItems( )  {
...
DataStoreItem*
GaudiPython::TESSerializer::findItem(const std::string& path)  {
  for(Items::const_iterator i=m_itemList.begin(); i != m_itemList.end(); ++i)  {
